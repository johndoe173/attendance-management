...
@section('content')
<h1 class="user-attendance-title">{{ $user->name }}さんの勤怠情報</h1>
<h2 class="date-nav">
    <a href="{{ route('user.show', ['id' => $user->id, 'date' => \Carbon\Carbon::parse($date)->subDay()->toDateString()]) }}">＜</a>
    {{ $date }}
    <a href="{{ route('user.show', ['id' => $user->id, 'date' => \Carbon\Carbon::parse($date)->addDay()->toDateString()]) }}">＞</a>
</h2>
